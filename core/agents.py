...
from services.swarm.forge_types import Task, AgentContract  # Contracts (MemoryScope unused)
...
try:
    from core.master_agent_coordinator import master_coordinator, create_intelligent_agent
    from core.enhanced_agent_intelligence import EnhancedAgentIntelligence, AgentRole
...
async def create_enhanced_agent_with_intelligence(
    role: str = "generalist",
    ...
    if ENHANCED_INTELLIGENCE_AVAILABLE:
        return await create_intelligent_agent(role, specializations)
    else:
        # Fallback to basic agent creation
        return create_basic_agent(role, specializations, agent_id)
...
def create_basic_agent(
    ...
    """Create basic agent (fallback when enhanced intelligence not available)"""
    import uuid
    if agent_id is None:
        agent_id = f"basic_{role}_{str(uuid.uuid4())[:8]}"
    
    # Create basic agent contract
    ...
    def process(self, task: Task) -> str:
        ...
        try:
            ...
            if self.memory is None:
                from services.swarm.memory import EvoMemory  # Lazy import to avoid slow startup
