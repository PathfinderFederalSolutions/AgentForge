...
from ..observability.task_latency import record_task_completion  # type: ignore
...
try:
    ...
    CHAT_ENDPOINTS_AVAILABLE = True
except ImportError as e:
    print(f"Chat endpoints not available: {e}")
    CHAT_ENDPOINTS_AVAILABLE = False

# Import Universal I/O endpoints
try:
    from .universal_io_endpoints import router as universal_io_router
    UNIVERSAL_IO_ENDPOINTS_AVAILABLE = True
except ImportError as e:
    print(f"Universal I/O endpoints not available: {e}")
    UNIVERSAL_IO_ENDPOINTS_AVAILABLE = False

# Import job management endpoints
try:
    from .job_management_endpoints import router as job_router
    JOB_ENDPOINTS_AVAILABLE = True
except ImportError as e:
    print(f"Job management endpoints not available: {e}")
    JOB_ENDPOINTS_AVAILABLE = False

# Import real-time endpoints
try:
    from .realtime_endpoints import router as realtime_router
    REALTIME_ENDPOINTS_AVAILABLE = True
except ImportError as e:
    print(f"Real-time endpoints not available: {e}")
    REALTIME_ENDPOINTS_AVAILABLE = False

# Import Phase 3 advanced intelligence endpoints
try:
    from .emergent_intelligence_endpoints import router as emergent_router
    EMERGENT_INTELLIGENCE_AVAILABLE = True
except ImportError as e:
    print(f"Emergent intelligence endpoints not available: {e}")
    EMERGENT_INTELLIGENCE_AVAILABLE = False

try:
    from .predictive_modeling_endpoints import router as predictive_router
    PREDICTIVE_MODELING_AVAILABLE = True
except ImportError as e:
    print(f"Predictive modeling endpoints not available: {e}")
    PREDICTIVE_MODELING_AVAILABLE = False

try:
    from .cross_modal_endpoints import router as cross_modal_router
    CROSS_MODAL_AVAILABLE = True
except ImportError as e:
    print(f"Cross-modal endpoints not available: {e}")
    CROSS_MODAL_AVAILABLE = False

try:
    from .self_improvement_endpoints import router as self_improvement_router
    SELF_IMPROVEMENT_AVAILABLE = True
except ImportError as e:
    print(f"Self-improvement endpoints not available: {e}")
    SELF_IMPROVEMENT_AVAILABLE = False

try:
    from .enterprise_management_endpoints import router as enterprise_router
    ENTERPRISE_MANAGEMENT_AVAILABLE = True
except ImportError as e:
    print(f"Enterprise management endpoints not available: {e}")
    ENTERPRISE_MANAGEMENT_AVAILABLE = False

# Optional tracing provider init
try:
    from ..observability.otel import init_tracing as _init_tracing
except Exception:  # pragma: no cover
    _init_tracing = None  # type: ignore

# Optional protocol models
try:
    from ..protocol.messages import TaskSpec  # canonical task spec
except Exception:
    TaskSpec = None  # type: ignore

# Optional observability context
try:
    from ..observability.costs import set_observability_context as _set_obs_ctx  # type: ignore
except Exception:  # pragma: no cover
    _set_obs_ctx = None  # type: ignore

# Optional default observable context
try:
    from ..observability.otel import set_default_observable_context as _set_default_ctx  # type: ignore
except Exception:  # pragma: no cover
...
# JetStream helpers
try:
    from ..jetstream import publish_job, ensure_streams
except Exception:
    publish_job = None
    ensure_streams = None
...
try:
    from ..memory.mesh_dist import DistMemoryMesh, start_snapshot_task
except Exception:
    DistMemoryMesh = None
    start_snapshot_task = None

try:  # Added import for fused track retrieval
    from ..storage import load_fused_track  # type: ignore
except Exception:  # pragma: no cover
...
def _sync_execute(goal: str, agents: int, job_id: str) -> Dict[str, Any]:
    ...
    try:
        from ..reviewer import review_results  # lazy import
...
# Include chat endpoints if available
if CHAT_ENDPOINTS_AVAILABLE:
    app.include_router(chat_router)
    print("Chat endpoints registered successfully")

# Include Universal I/O endpoints if available
if UNIVERSAL_IO_ENDPOINTS_AVAILABLE:
    app.include_router(universal_io_router)
    print("Universal I/O endpoints registered successfully")

# Include job management endpoints if available
if JOB_ENDPOINTS_AVAILABLE:
    app.include_router(job_router)
    print("Job management endpoints registered successfully")

# Include real-time endpoints if available
if REALTIME_ENDPOINTS_AVAILABLE:
    app.include_router(realtime_router)
    print("Real-time endpoints registered successfully")

# Include Phase 3 advanced intelligence endpoints
if EMERGENT_INTELLIGENCE_AVAILABLE:
    app.include_router(emergent_router)
    print("Emergent intelligence endpoints registered successfully")

if PREDICTIVE_MODELING_AVAILABLE:
    app.include_router(predictive_router)
    print("Predictive modeling endpoints registered successfully")

if CROSS_MODAL_AVAILABLE:
    app.include_router(cross_modal_router)
    print("Cross-modal understanding endpoints registered successfully")

if SELF_IMPROVEMENT_AVAILABLE:
    app.include_router(self_improvement_router)
    print("Self-improvement endpoints registered successfully")

if ENTERPRISE_MANAGEMENT_AVAILABLE:
    app.include_router(enterprise_router)
    print("Enterprise management endpoints registered successfully")
