...
export const AlertsLayer: React.FC<{ items: EventFeature[]; onSelect: (f: EventFeature)=>void }> = ({ items, onSelect }: { items: EventFeature[]; onSelect: (f: EventFeature)=>void }) => {
  return (
    <>
      {items.map((f: EventFeature, idx: number) => {
        ...
        const title = String(f.properties?.title ?? 'Alert')
        const evid = f.properties?.evidence_link as string | undefined
        const evidSafe = sanitizeUrl(evid)
        return (
          <Marker key={`a-${idx}`} position={[lat, lon]} icon={blueIcon} eventHandlers={{ click: () => onSelect(f) }}>
            <Popup>
              <div>
                <strong>{title}</strong>
